...
pub fn build(b: *std.Build) !void {
    ...
    var as_buf = try std.Io.Writer.Allocating.initCapacity(b.allocator, 16 * 1024);
    defer as_buf.deinit();
    var asw = as_buf.writer;
    try asw.writeAll("pub const Applet = struct { name: []const u8, help: []const u8, main: *const fn (args: [][:0]u8) anyerror!u8 };\npub fn load_applet(comptime name: []const u8, root: type) Applet {\n    return .{ .name = name, .help = root.help, .main = root.main };\n}\npub var _ap = [_]Applet{\n");
    ...
    while (try dir.next()) |entry| {
        ...
        try asw.print("\tload_applet(\"{s}\", @import(\"applets/{s}\")),\n", .{ entry.name[0 .. entry.name.len - 4], entry.name });
    }
    try asw.writeAll("\n}};\n");
    const write_applets = b.addWriteFile(b.pathFromRoot("applets.zig"), try as_buf.toOwnedSlice());
