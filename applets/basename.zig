...
const Parser = argp.Gen(.{val("suffix", true, "Removes the suffix from each of following paths.")} ++ argp.std_vh, .{}, .{ .usage = usage, .desc = desc });
pub fn main(args: [][:0]u8) !u8 {
    ...
    while (parser.nextArg()) |arg| {
        ...
        switch (arg) {
            ...
            .flag_args => |f| {
                ...
                if (idx != 1) die(1, "Suffix flag may only be provided as the first argument.", .{});
                if (value.len == 0) die(1, "Specified an empty suffix.", .{});
                suffix = value;
            },
            .positional => break,
        }
    ...
    if (iargs.len >= 3) die(1, "Extra arguments '{s}' and beyond.", .{iargs[2]});
