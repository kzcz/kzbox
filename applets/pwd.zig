...
pub fn main(args: [][:0]u8) !u8 {
    const environ = root.init.environ_map;
    const out = root.out;
    ...
    if (flag == .L) blk: {
        const pwd_env = (environ.getPtr("PWD") orelse break :blk).*;
        if (pwd_env.len > 4096 or pwd_env.len == 0 or pwd_env[0] != '/') break :blk;
        var penv: []const u8 = pwd_env;
        while (true) {
    ...
    const size = std.process.currentPath(root.init.io, &outbuf) catch |err| root.dieErr(1, "getcwd", err);
    var path: []const u8 = lib.scalarTrimEnd(outbuf[0..size], '/');
